...
if __name__ == "__main__": 
    ...
    for j in range(3):
        ...
        for i in range(0,21):
            ...
            data.Draw("tripletMass>>h_sig" +s+ "(52, 1.6, 2)", "combine_weight*(isMC>0 && abs(1.777 - tripletMass)<"+ str(3*sigma_tau[j]) + "&& category=="+ str(j) + "&& abs(dimu_OS1 - 0.7826)>"+str((i/2)*sigma_omega[j])  + "&& abs(dimu_OS2 - 0.7826)>"+str((i/2)*sigma_omega[j])  +")")
            ...
            #data.Draw("tripletMass>>h_sig" +s+ "(52, 1.6, 2)", "combine_weight*(isMC>0 && abs(1.777 - tripletMass)<"+ str(3*sigma_tau[j]) + "&& category=="+ str(j) + "&& abs(dimu_OS1 - 1.0195)>"+str((i/4)*sigma_phi[j])  + "&& abs(dimu_OS2 - 1.0195)>"+str((i/4)*sigma_phi[j])  +")")
