...
if __name__ == "__main__": 
    ...
    for j in range(3):
        ...
        for i in range(1,11):
            s = str((j+1)*i)
            data.Draw("tripletMass>>hbkg" +s+ "(52, 1.6, 2)", "(isMC==0 && abs(1.777 - tripletMass)>"+ str(3*sigma_tau[j]) + "&& category=="+j + "&& abs(dimu_OS1 - 1.0195)>"str((i/2)*sigma_phi[j])  + "&& abs(dimu_OS2 - 1.0195)>"str((i/2)*sigma_phi[j])  +")")
            data.Draw("tripletMass>>hsig" +s+ "(52, 1.6, 2)", "combine_weight*(isMC>0" + "&& category=="+j + "&& abs(dimu_OS1 - 1.0195)>"str((i/2)*sigma_phi[j])  + "&& abs(dimu_OS2 - 1.0195)>"str((i/2)*sigma_phi[j])  +")")
